...
import launcher.Launcher.GameState;
import model.AI;
import model.Player;
...
public class RankingView extends JPanel {
    ...
    public RankingView(Window window, Player[] players) {
        ...
        for(Player p : players) {
            if(!(p instanceof AI) && p.getRank() == 1) {
                BoardGame.victoryCount++;
            }
            if(p instanceof AI)
                ai=true;
        }
        // Si au moins 1 IA dans la partie on valide. Sinon on remet le compteur de victoire a 0.
        if(!ai)
            BoardGame.victoryCount = 0;
        ...
        GameState.gethelp().checkFlagConditions();
    }
