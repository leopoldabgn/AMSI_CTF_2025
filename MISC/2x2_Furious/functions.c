...
{
    if(position >= 200) {
        printf("\nERROR: AI exceeded 200 moves without solving the cube. Giving up.");
        exit(EXIT_SUCCESS);
    }
...
int is_cube_solved()
{
    // Vérifie chaque face 2x2 pour couleur uniforme

    // Top face (indices [0][0] à [1][1])
    int top = cube[0][0];
    if (cube[0][1] != top || cube[1][0] != top || cube[1][1] != top)
        return 0;

    // Left face (indices [2][0], [2][1], [3][0], [3][1])
    int left = cube[2][0];
    if (cube[2][1] != left || cube[6][0] != left || cube[6][1] != left)
        return 0;

    // Front face (indices [2][2], [2][3], [3][2], [3][3])
    int front = cube[3][0];
    if (cube[3][1] != front || cube[7][0] != front || cube[7][1] != front)
        return 0;

    // Right face (indices [2][4], [2][5], [3][4], [3][5])
    int right = cube[4][0];
    if (cube[4][1] != right || cube[8][0] != right || cube[8][1] != right)
        return 0;

    // Back face (indices [2][6], [2][7], [3][6], [3][7])
    int back = cube[5][0];
    if (cube[5][1] != back || cube[9][0] != back || cube[9][1] != back)
        return 0;

    // Bottom face (indices [4][0], [4][1], [5][0], [5][1])
    int bottom = cube[10][0];
    if (cube[10][1] != bottom || cube[11][0] != bottom || cube[11][1] != bottom)
        return 0;

    // Si toutes les faces sont uniformes
    return 1;
}
